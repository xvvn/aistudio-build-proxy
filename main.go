...
// validateJWT 验证JWT token并返回userID
func validateJWT(token string) (string, error) {
    if token == "" {
        return "", errors.New("missing auth_token")
    }
    
    // 验证token是否与启动时提供的apiKey匹配
    if token == apiKey {
...
// authenticateHTTPRequest 验证HTTP代理请求的认证
...
func init() {
    // 初始化命令行参数
    flag.StringVar(&apiKey, "key", "", "API key for authentication (required)")
    flag.StringVar(&listenAddr, "addr", proxyListenAddr, "Address to listen on")
    flag.Parse()
    
    // 验证必需参数
    if apiKey == "" {
        log.Fatal("Error: API key is required. Use -key flag to provide it.")
    }
}
...
func main() {
    ...
    log.Printf("WebSocket endpoint available at ws://%s%s", listenAddr, wsPath)
    log.Printf("HTTP proxy available at http://%s/", listenAddr)
